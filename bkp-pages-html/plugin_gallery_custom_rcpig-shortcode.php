<?php
/*
 *  Responsive Portfolio Image Gallery 1.0.6
...
function rcpig_shortcode_function( $atts ){
	...
	$rcpig_cat_excludes = rcpig_get_option( 'rcpig_cat_exclude_', 'rcpig_general', array('') );
	$rcpig_taxonomy = 'rcpig-category';
	if( isset($rcpig_cat_excludes) && is_array($rcpig_cat_excludes) ){
		foreach ($rcpig_cat_excludes as $rcpig_cat_exclude_key => $rcpig_cat_exclude_value) {
			$rcpig_cats_ex[] = $rcpig_cat_exclude_key;
		}
	}
				...
				jQuery(function(){
					jQuery("#rcpig_grid_demo").rcpig_grid({
						...
						while ( $loop->have_posts() ) : $loop->the_post();
							...
							$image_thumb = aq_resize( $img_url, $rcpig_image_width, $rcpig_image_height, true, true, true ); //resize & crop the image
							...
							$rcpig_button_text = get_post_meta( $post->ID, '_first_button', true ) != '' ? get_post_meta( $post->ID, '_first_button', true ) : 'Download';
							$rcpig_button_link = get_post_meta( $post->ID, '_first_button_link', true ) != '' ? get_post_meta( $post->ID, '_first_button_link', true ) : '#';
							$rcpig_button_tab = get_post_meta( $post->ID, '_first_button_tab', true ) != '' ? get_post_meta( $post->ID, '_first_button_tab', true ) : 'false'; ?>
							{
								...
								'tags'          : ['<?php foreach($terms as $term) { if(!in_array($term->name, $rcpig_cat_excludes)) { echo str_replace(' ','-', strtolower($term->name)); } } ?>']
							},<?php
						endwhile; ?>
						]
					});
...
//add_action('after_setup_theme', 'rcpig_shortcode_function');
?>